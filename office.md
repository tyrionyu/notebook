...
![image-20250623094650433](./office.assets/image-20250623094650433.png)

图书租借，车辆归还/未归还...可以用时间来差来判断

![image-20250719202312928](office.assets/image-20250719202312928.png)
...
1、选项-高级-取消勾选“使用智能段落选择”
2、选项-校对-键入时自动套用格式-取消勾选“用tab和退格设置左缩进和首行缩进”
3、段落-中文版式-勾选 允许西文在单词中换行
4、文件-选项-校对-自动更正-取消勾选“自动编号列表“
5、文件-选项-校对-键入时自动套用格式-取消勾选”指引号替换为弯引号“
6、选项-高级-显示文档内容-取消勾选 显示裁剪标记
...
第三步:查找:^g^p 格式替换:字体颜色紫色

第四步:查找:^p(字体颜色紫色)替换:^p^c(字体黑色)
...
## 在WPS多维表中删除下拉列表选项的操作建议

### 方案A：直接删除并更新历史记录​​

- 编辑下拉列表，删除不需要的选项
- 使用筛选功能找出所有包含该选项的记录
- 批量更新这些记录，选择新的有效选项或设为空值
...
### 方案C：新开一表，使用合并功能进行合并，并成一个总表

### 注意事项
- 直接删除选项可能导致历史记录显示异常（可能显示为空白或错误值）
- 如果该选项对数据分析很重要，考虑先添加新选项并迁移数据后再删除
- 对于关键业务数据，建议先在测试环境中验证操作影响